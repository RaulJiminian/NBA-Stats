...
export default function BarGraphRebounds({ playerAverages }) {
  ...
  useEffect(() => {
    ...
    yScale.domain([0, max(playerAverages, (d) => d.stats[0].rebounds) + 5]);
    ...
    chart
      ...
      .attr("height", (data) => CHART_HEIGHT - yScale(data.stats[0].rebounds))
      .attr("x", (data) => xScale(data.year))
      .attr("y", (data) => yScale(data.stats[0].rebounds));

    chart
      .selectAll(".label")
      .data(playerAverages)
      .join("text")
      .text((data) => data.stats[0].rebounds)
      .attr("x", data => xScale(data.year) + xScale.bandwidth() / 2)
      .attr("y", data => yScale(data.stats[0].rebounds) - 5)
