...
export default function BarGraphSteals({ playerAverages }) {
  ...
  useEffect(() => {
    ...
    yScale.domain([0, max(playerAverages, (d) => d.stats[0].steals) + 5]);
    ...
    chart
      ...
      .attr("height", (data) => CHART_HEIGHT - yScale(data.stats[0].steals))
      .attr("x", (data) => xScale(data.year))
      .attr("y", (data) => yScale(data.stats[0].steals));

    chart
      .selectAll(".label")
      .data(playerAverages)
      .join("text")
      .text((data) => data.stats[0].steals)
      .attr("x", data => xScale(data.year) + xScale.bandwidth() / 2)
      .attr("y", data => yScale(data.stats[0].steals) - 5)
